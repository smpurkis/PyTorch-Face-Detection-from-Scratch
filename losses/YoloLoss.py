...
class YoloLoss:
    def __init__(self, non_max_suppression_fn):
        self.non_max_suppression_fn = non_max_suppression_fn

    def __call__(self, pred_fm, gt_fm):
        # gt_bbx = self.non_max_suppression_fn(gt_fm).reshape(-1, 5)[:, 1:]
        # pred_bbx = self.non_max_suppression_fn(pred_fm).reshape(-1, 5)[:, 1:]
